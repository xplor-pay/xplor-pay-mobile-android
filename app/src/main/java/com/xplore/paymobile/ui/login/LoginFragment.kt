...
class LoginFragment : Fragment() {
    ...
    companion object {
        ...
        fun newInstance(onLoginSuccessful: () -> Unit) = LoginFragment().apply {
            arguments = bundleOf(ON_LOGIN_SUCCESSFUL_KEY to OnSuccessfulLogin(onLoginSuccessful))
        }

        @Parcelize
        data class OnSuccessfulLogin(val data: () -> Unit) : Parcelable
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        arguments?.parcelable<OnSuccessfulLogin>(ON_LOGIN_SUCCESSFUL_KEY)?.data?.also {
